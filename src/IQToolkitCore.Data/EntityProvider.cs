...
namespace IQToolkit.Data
{
    ...
    public abstract class EntityProvider : QueryProvider, IEntityProvider, IQueryExecutorFactory
    {
        ...
        /// クエリを実行するためにデータベースに送信される個々のコマンドのテキストを取得します。
        /// クライアント側のプロジェクションまたはオーケストレーション ロジックは含まれません。
        /// </summary>
        ...
        /// 
        /// クエリ式を実行プランに変換します。
        /// これは、サーバーに対して発行されるすべての QueryCommand と、
        /// 返されたデータを期待される要素タイプに変換するためにクライアントで実行するロジックを含む単一の式です。
        /// </summary>
